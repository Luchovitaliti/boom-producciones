...
// Uses _ensureBen from liderpub.js for write path
function bEntregaBtn(pid,key,active,entregado){
  if(!active)return'<span style="color:var(--text2);font-size:11px">—</span>';
  const ts=entregado[key]||'';
  return ts
...
function bLoadBeneficios(){
  ...
  pubs.forEach(p=>{
    // Read-only copy per pública — no shared references
    ...
    h+=`<tr>
      ...
      <td style="text-align:center">${bEntregaBtn(p.id,'ent',b.ent,b.entregado)}</td>
      <td style="text-align:center">${bEntregaBtn(p.id,'con',b.conCant&&b.conTipo,b.entregado)}</td>
      <td style="text-align:center">${bEntregaBtn(p.id,'ex',b.ex,b.entregado)}</td>
    </tr>`;
  });
  h+=`</tbody></table></div>`;
...
function bTogEntrega(pid,key){
  ...
  b.entregado[key]=b.entregado[key]?'':ts;
