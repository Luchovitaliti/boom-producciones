...
function pgUsuarios() {
  ...
  h += `<div style="margin-bottom:1.25rem">
    <div style="font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:.07em;font-weight:500;margin-bottom:10px">Activos (${USERS.length})</div>`;
  USERS.forEach((u, i) => {
    if (!u) return;
    ...
    const isAdmin = role === 'Admin Console';
    h += `<div class="usr-card">
      <div class="usr-card-top">
        ${avatarHtml(photo, uid, i, 44)}
        <div class="usr-card-info">
          <div class="usr-card-name">${escapeHtml(uid)}</div>
          <div class="usr-card-sub">${escapeHtml(cname)||'—'} · <span style="color:var(--accent)">${escapeHtml(role)}</span></div>
          ${email ? `<div style="font-size:10px;color:var(--text3);margin-top:2px">${escapeHtml(email)}</div>` : ''}
        </div>
      </div>
      <div class="usr-card-tags">${pages.map(p => `<span class="usr-card-tag">${PAGE_ICONS[p]||''} ${PAGE_LABELS[p]||p}</span>`).join('')}</div>
      <div class="usr-card-actions">
        <button class="btn btnp" onclick="openUserEdit(${i})">✏️ Editar</button>
        ${!isAdmin ? `<button class="btn btnd" onclick="delUser(${i})">Eliminar</button>` : ''}
      </div>
    </div>`;
  });
  h += `</div>`;
  ...
  h += `<div class="card" style="margin-bottom:1rem">
    <div class="ctitle">➕ Agregar usuario</div>
    <div class="fc" style="margin-bottom:8px"><span class="fl">Usuario</span>
      <input type="text" id="nu-u" style="width:100%" placeholder="Ej: FOTO" oninput="this.value=this.value.toUpperCase()"></div>
    <div class="fc" style="margin-bottom:8px"><span class="fl">Email</span>
      <input type="email" id="nu-email" style="width:100%" placeholder="correo@real.com"></div>
    <div class="fc" style="margin-bottom:8px"><span class="fl">Contraseña</span>
      <input type="password" id="nu-p" style="width:100%" placeholder="Mínimo 6 caracteres"></div>
    <div class="fc" style="margin-bottom:8px"><span class="fl">Nombre en chat</span>
      <input type="text" id="nu-cn" style="width:100%" placeholder="Ej: Fotógrafo"></div>
    <div class="fc" style="margin-bottom:12px"><span class="fl">Rol</span>
      <select id="nu-r" style="width:100%">
    ...
    <button class="btn btnp" style="width:100%;justify-content:center;padding:11px 0;font-size:13px;border-radius:14px" onclick="addUser()" id="nu-btn">Agregar usuario</button>
    <div id="u-msg" style="font-size:12px;margin-top:8px;line-height:1.5;text-align:center"></div>
  </div>`;

  // ── Módulos disponibles (iOS icon grid) ──
  h += `<div class="card" style="margin-bottom:1rem">
    <div class="ctitle">📦 Módulos disponibles</div>
    <div style="font-size:11px;color:var(--text2);margin-bottom:14px">Asigná módulos desde el botón <strong>Editar</strong> de cada usuario.</div>
    <div class="usr-app-grid">
      ${ALL_PAGES.map(p => `
        <div class="usr-app-icon">
          <div class="usr-app-icon-box">${PAGE_ICONS[p]||'📦'}</div>
          <span class="usr-app-icon-label">${PAGE_LABELS[p]||p}</span>
        </div>`).join('')}
    </div>
  </div>`;
  ...
  h += `<div class="card">
    ...
    <div style="font-size:12px;color:var(--text2);margin-bottom:10px">Detecta usuarios en Firestore cuya cuenta de Firebase Auth ya no existe y los desactiva.</div>
    <button class="btn btnsm" id="orphan-btn" onclick="cleanOrphans()" style="width:100%;justify-content:center;border-radius:12px;padding:9px 0">Limpiar huérfanos</button>
    <div id="orphan-msg" style="font-size:12px;margin-top:8px;min-height:14px;text-align:center"></div>
  </div>`;
...
function openUserEdit(idx) {
  ...
  document.getElementById('m-usr-edit-body').innerHTML = `
    ...
    <div class="fc" style="margin-bottom:8px"><span class="fl">Rol</span>
      <select id="ue-role" style="width:100%" ${isAdmin ? 'disabled' : ''}>
        ${roles.map(r => `<option value="${r}" ${u.role === r ? 'selected' : ''}>${r}</option>`).join('')}
      </select>
    </div>
    <div class="fc" style="margin-bottom:8px"><span class="fl">Email</span>
      <input type="text" id="ue-email" style="width:100%;color:var(--text3)" value="${escapeHtml(u.email || '')}" readonly></div>
    <div class="fc" style="margin-bottom:12px"><span class="fl">Username</span>
    ...
    <div style="font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:.07em;font-weight:500;margin-bottom:8px">Módulos</div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:1rem">
      ${ALL_PAGES.filter(p => p !== 'usuarios' || isAdmin).map(p => {
        ...
        return `<div style="display:flex;align-items:center;gap:8px;padding:7px 10px;border-radius:12px;background:var(--glass2);border:1px solid var(--glass-border)">
          <button class="chkbtn ${on ? 'on' : ''}" id="umod-${p}" onclick="toggleUmod('${p}')">${on ? '✓' : ''}</button>
          <span style="font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${PAGE_ICONS[p] || ''} ${PAGE_LABELS[p] || p}</span>
        </div>`;
      }).join('')}
    </div>
    ${!isAdmin ? `
    <div style="background:rgba(248,113,113,.04);border:1px solid rgba(248,113,113,.12);border-radius:16px;padding:12px;margin-top:4px">
      <div style="font-size:12px;font-weight:600;margin-bottom:8px">🔐 Restablecer contraseña</div>
      <div style="font-size:11px;color:var(--text2);margin-bottom:10px">Se cambia directamente en Firebase Auth.</div>
      <div class="fc" style="margin-bottom:8px"><span class="fl">Nueva contraseña</span>
        <input type="password" id="ue-pass1" style="width:100%" placeholder="Mínimo 6 caracteres"></div>
      <div class="fc" style="margin-bottom:8px"><span class="fl">Confirmar</span>
        <input type="password" id="ue-pass2" style="width:100%" placeholder="Repetir contraseña"></div>
      <button class="btn btnsm" style="width:100%;justify-content:center;border-radius:12px;padding:9px 0" onclick="resetUserPass(${idx})">🔑 Cambiar contraseña</button>
      <div id="ue-pass-msg" style="font-size:12px;margin-top:6px;min-height:14px;text-align:center"></div>
    </div>` : ''}
  `;
